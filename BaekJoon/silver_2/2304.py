...
# 이 그림에서 굵은 선으로 표시된 부분이 지붕에 해당되고, 지붕과 땅으로 둘러싸인 다각형이 창고를 옆에서 본 모습이다. 이 다각형을 창고 다각형이라고 하자.
...
mh_li = []
mh = max(di.values())
[mh_li.append(k) for k,v in di.items() if mh == v]
...
for i in range(sh, lh):
    if i in di and di[i] > lmh:
        lmh = di[i]
    left += lmh

for i in range(eh, rh-1, -1):
    if i in di and di[i] > rmh:
        rmh = di[i]
    right += rmh
