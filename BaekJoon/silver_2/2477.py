...
# 남쪽으로 20m, 동쪽으로 100m, 북쪽으로 50m,서쪽으로 160m 이동하면 다시 출발점으로 되돌아가게 된다.
...
li = [list(map(int, sys.stdin.readline().split())) for _ in range(6)]
ml = [0, 0]
...
li *= 2
for i in range(12):
    if li[i][1] in ml and li[i+1][1] in ml:
        ss = li[i+3][1]*li[i+4][1]
        break
print((bs-ss)*n)