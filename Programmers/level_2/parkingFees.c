...
int* solution(int fees[], size_t fees_len, const char* records[], size_t records_len) {
    ...
    for(int i=0; i<records_len; i++){
        int c = (records[i][0]-'0')*10 + records[i][1]-'0';
        int m = (records[i][3]-'0')*10 + records[i][4]-'0';
        int clk = c * 60 + m;
        
        int num = 0, pos = 1000;
        for(int j=6; j<10; j++){
            num += (records[i][j]-'0')*pos;
            pos /= 10;
        }
        ...
        if(records[i][11] == 'I'){
            ...
            park[num] = false;
            useTime[num] += clk - startTime[num];
        }
    }
    
    for(int i=0; i<10000; i++)
        if(park[i]) useTime[i] += 23*60+59 - startTime[i];
    ...
    for(int i=0; i<10000; i++){
        if(useTime[i] > 0){
            if(fees[0] >= useTime[i])
                ...
                if((useTime[i]-fees[0])%fees[2] != 0)
                    answer[j++] = fees[1]+fees[3]*((useTime[i]-fees[0])/fees[2]+1);
                else
                    answer[j++] = fees[1]+fees[3]*(useTime[i]-fees[0])/fees[2];
            }
        }    
    }
    return answer;
