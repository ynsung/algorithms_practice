...
def solution(bandage, health, attacks):
    HP = health
    t, hps, add_heal = bandage[0], bandage[1], bandage[2]
    fin_attack = attacks[len(attacks)-1][0]
    dmg = {}
    for i in attacks:
        dmg[i[0]] = i[1]
    
    cnt = 0
    for i in range(1, fin_attack+1):
        if i in dmg:
            HP -= dmg[i]
            if HP <= 0:
                return -1
            ...
            cnt += 1
            HP = HP+hps if HP+hps<=health else health
            if cnt==t:
                HP = HP+add_heal if HP+add_heal<=health else health
                cnt = 0
    return HP